...
export default function ClientLendings() {
    ...
    const { lendings, lendingsFilter, loadingGotchis, sortData } = useContext(ClientContext);

    if (loadingGotchis || !lendings.length) {
        return <Box className={classes.loaderBox}>
            <GhostLoader
                animate={loadingGotchis || !lendings.length}
                text={!loadingGotchis && !lendings.length ? 'No gotchis here :(' : null}
            />
        </Box>
    }
