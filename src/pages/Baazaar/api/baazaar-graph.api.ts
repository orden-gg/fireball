...
export class BaazaarGraphApi {
  public static async getErc721Listings<T>(query: string): Promise<T[]> {
    return TheGraphCoreApi.getGraphData(GRAPH_CORE_API, query).then(
      (res: TheGraphResponse<{ erc721Listings: T[] }>) => res.data.erc721Listings
    );
  }

  public static async getParcelListings<T>(query: string): Promise<T[]> {
    return TheGraphCoreApi.getGraphData(GRAPH_FIREBALL_API, query).then(
      (res: TheGraphResponse<{ erc721Listings: T[] }>) => res.data.erc721Listings
    );
  }
  ...
  public static async getErc1155Purchases<T>(query: string): Promise<T[]> {
    return TheGraphCoreApi.getGraphData(GRAPH_CORE_API, query).then(
      (res: TheGraphResponse<{ erc1155Purchases: T[] }>) => res.data.erc1155Purchases
    );
  }
}
