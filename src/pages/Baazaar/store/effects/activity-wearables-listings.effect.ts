...
export const updateActivityWearablesListingsFilterByKey =
    ({ key, value }: { key: ActivityWearableListingFilterTypes, value: GraphFiltersValueTypes }): AppThunk =>
        (dispatch, getState) => {
            ...
            dispatch(setActivityWearablesListingsFilters({ ...filters, [key]: updatedFilter as ActivityWearableListingFiltersType }));
        };

export const resetActivityWearablesListingsFilters = (): AppThunk =>
    (dispatch, getState) => {
        const filters: ActivityWearableListingFilters = getState().baazaar.activity.wearables.activityWearablesListingsFilters;
...
export const resetActivityWearablesData = (): AppThunk =>
    (dispatch, getState) => {
        const filters: ActivityWearableListingFilters = getState().baazaar.activity.wearables.activityWearablesListingsFilters;
