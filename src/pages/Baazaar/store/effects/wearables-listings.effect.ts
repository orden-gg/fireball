  ...
  (dispatch, getState) => {
    ...
    BaazaarGraphApi.getErc1155Listings<WearableListingDTO>(query)
      .then((wearablesListings: WearableListingDTO[]) => {
        const wearablesIds: number[] = wearablesListings.map((listing: WearableListingDTO) =>
        ...
        if (wearablesIds.length > 0) {
          TheGraphApi.getErc1155ListingsBatchQuery(
            ...
            .then((lastSoldListings: Erc1155ListingsBatch) => {
              const modifiedListings: WearableListingVM[] = mapWearablesListingsDTOToVM(
                wearablesListings,
                lastSoldListings
              );
...
export const onSetWearablesListingsSorting =
