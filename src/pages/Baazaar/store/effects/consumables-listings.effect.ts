  ...
  (dispatch, getState) => {
    ...
    BaazaarGraphApi.getErc1155Listings<ConsumableListingDTO>(query)
      .then((consumablesListings: ConsumableListingDTO[]) => {
        const consumablesIds: number[] = consumablesListings.map((listing: ConsumableListingDTO) =>
        ...
        if (consumablesIds.length > 0) {
          TheGraphApi.getErc1155ListingsBatchQuery(
            ...
            .then((lastSoldListings: Erc1155ListingsBatch) => {
              const modifiedListings: ConsumableListingVM[] = mapConsumablesListingsDTOToVM(
                consumablesListings,
                lastSoldListings
              );
...
export const onSetConsumablesListingsSorting =
