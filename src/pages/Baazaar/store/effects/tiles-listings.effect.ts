  ...
  (dispatch, getState) => {
    ...
    BaazaarGraphApi.getErc1155Listings<TileListingDTO>(query)
      .then((tilesListings: TileListingDTO[]) => {
        const tilesIds: number[] = tilesListings.map((listing: TileListingDTO) => Number(listing.erc1155TypeId));
...
export const onSetTilesListingsSorting =
