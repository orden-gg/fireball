...
export const loadBaazaarActivityConsumablesListings = (): AppThunk => (dispatch, getState) => {
    ...
    const filters: ActivityConsumableListingFilters = getState().baazaar.activity.consumables.activityConsumablesListingsFilters;
...
export const updateActivityConsumablesListingsFilterByKey =
    ({ key, value }: { key: ActivityConsumableListingFilterTypes, value: GraphFiltersValueTypes }): AppThunk =>
        (dispatch, getState) => {
            ...
            dispatch(setActivityConsumablesListingsFilters({ ...filters, [key]: updatedFilter as ActivityConsumableListingFiltersType }));
        };

export const resetActivityConsumablesListingsFilters = (): AppThunk =>
    (dispatch, getState) => {
        const filters: ActivityConsumableListingFilters = getState().baazaar.activity.consumables.activityConsumablesListingsFilters;
...
export const resetActivityConsumablesData = (): AppThunk =>
    (dispatch, getState) => {
        const filters: ActivityConsumableListingFilters = getState().baazaar.activity.consumables.activityConsumablesListingsFilters;
