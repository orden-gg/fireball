...
export function BaazaarActivityConsumables() {
  ...
  const activityConsumablesListings: ActivityConsumableListingVM[] = useAppSelector(
    fromBaazaarStore.getActivityConsumablesListings
  );
  const isActivityConsumablesListingsInitialDataLoading: boolean = useAppSelector(
    fromBaazaarStore.getIsActivityConsumablesListingsInitialDataLoading
  );
  const isActivityConsumablesListingsLoading: boolean = useAppSelector(
    fromBaazaarStore.getIsActivityConsumablesListingsLoading
  );
  const activityConsumablesListingsFilters: ActivityConsumableListingFilters = useAppSelector(
    fromBaazaarStore.getActivityConsumablesListingsFilters
  );
  const activityConsumablesListingsQueryParamsOrder: string[] = useAppSelector(
    fromBaazaarStore.getActivityConsumablesListingsQueryParamsOrder
  );

  useEffect(() => {
    const updatedFilters: ActivityConsumableListingFilters = GraphFiltersUtils.getUpdatedFiltersFromQueryParams(
      queryParams,
  ...
  const onSetSelectedFilters = (key: string, value: GraphFiltersValueTypes) => {
    dispatch(
      fromBaazaarStore.updateActivityConsumablesListingsFilterByKey({ key, value } as {
        key: ActivityConsumableListingFilterTypes;
