...
export const addAddress = ({ address, name }: LoginAddress): AppThunk =>
    (dispatch, getState) => {
...
export const updateAddressName = (address: string, name: string): AppThunk =>
    (dispatch, getState) => {
        const state = getState();
...
export const removeAddress = (address: string): AppThunk =>
    (dispatch, getState) => {
        const state = getState();
        const filteredAddresses: LoginAddress[] = state.login.loggedAddresses.filter((item: LoginAddress) => item.address !== address);
...
export const selectActiveAddress = (address: string): AppThunk =>
    (dispatch) => {
