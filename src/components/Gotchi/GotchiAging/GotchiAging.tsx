...
export function GotchiAging({ block }: { block: number }) {
  ...
  useEffect(() => {
    ...
    Promise.all([EthersApi.getLastBlock(), EthersApi.getBlockByNumber(block)]).then(([lastBlock, birthdayBlock]) => {
    ...
    <div className={classes.container}>
      {!dataLoading && aging ? (
        <div className={classes.inner}>
          <div>
            <img src={GotchiUtils.getAgingImg(aging.metadata.boost)} alt={aging.metadata.name} />
          </div>
          <div>
            ...
            <div>~{CommonUtils.convertFloatNumberToSuffixNumber(aging.blockDiff)} blocks ago</div>
          </div>
        </div>
      ) : (
        <Skeleton className={classes.placeholder} variant='rectangular' width={280} height={66} />
      )}
    </div>
  );
}
