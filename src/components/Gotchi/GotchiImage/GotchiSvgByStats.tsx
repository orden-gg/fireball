...
export function GotchiSvgByStats({ gotchi, size }: GotchiSvgByStatsProps) {
    ...
    useEffect(() => {
        ...
        renderSvg([gotchi]).then((response: any) => {
            ...
            svgUniqueStyles = svgUniqueStyles[0].match(regexClass).map((styleBlock: any) => {
    ...
    return (
        <div className={classes.svgWrapper} style={{ width: size }}>
            {loadingSvg ? (
                ...
                <div
                    className={classNames(`gotchi-${gotchi.tempId}`, `gotchi-svg-${gotchi.id}`)}
                    ref={svgRef}
                ></div>
            )}
        </div>
