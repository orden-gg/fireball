...
export class FiltersManager {
  ...
  updateFilters(filters) {
    for (const [key, filter] of Object.entries(filters)) {
      if (this.filters.hasOwnProperty(key)) {
        const isChanged = filter.items.some(
          (item, index) => item.isSelected !== this.filters[key].items[index].isSelected
        );
  ...
  districtFilter() {
    ...
    for (const item of filter.items) {
      ...
      district.setAlpha(filter.isFilterActive && !item.isSelected ? 0.5 : 1);
    }
  }
  ...
  sizeFilter() {
    ...
    for (const key in this.parcelsFades) {
      ...
      this.parcelsFades[key] = filter.items[key].isSelected ? 1 : 0.2;
    }
  ...
  fadeMapParts() {
    const number =
      this.groups.isGroupsActive || Object.entries(this.filters).some(([, filter]) => filter.isFilterActive) ? 0.5 : 1;
    this.scene.walls.setAlpha(number);

    for (const [, alchemica] of Object.entries(this.scene.alchemica)) {
      alchemica.setAlpha(number === 1 ? 1 : 0.25);
    }
  }
