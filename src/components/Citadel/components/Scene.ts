...
export class CitadelScene extends Phaser.Scene {
  ...
  zoomToPointer(pointer) {
    ...
    const [cx, cy] = [centerX + -this.cursorFromCenter.cx + offsetX, centerY + -this.cursorFromCenter.cy + offsetY];
  ...
  zoomToDistrict(id) {
    ...
    this.moveToCenter(cx, cy, 500);
    this.zoomTo(this.getZoomBySize(w, h) * 0.9, 500);
    this.trigger('districtHover', id, this.settings.district);
    this.settings.district = id;
  }
