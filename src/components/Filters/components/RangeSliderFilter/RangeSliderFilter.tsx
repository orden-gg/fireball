...
export function RangeSliderFilter({ filter, onSetSelectedFilters }: RangeSliderFilterProps) {
    ...
    const onMinInputChange = useCallback((value: string) => {
        const numValue: number = Number(value);

        setMinValue(numValue);

        const predicate = value.match(floatNumberPattern) &&
            numValue >= filter.min && numValue <= filter.max && numValue <= maxValue;
    ...
    const onMaxInputChange = useCallback((value: string) => {
        const numValue: number = Number(value);

        setMaxValue(numValue);

        const predicate = value.match(floatNumberPattern) &&
            numValue >= filter.min && numValue <= filter.max && numValue >= minValue;
    ...
    const onSliderChangeCommited = useCallback((value: number[]) => {
