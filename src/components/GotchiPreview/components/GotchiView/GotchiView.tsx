...
export function GotchiView({ gotchi }: GotchiViewProps) {
    ...
    useEffect(() => {
        const wearables: number[] = gotchi.equippedWearables.filter((id: number) =>
            id !== 0 && ItemUtils.getSlotsById(id)[0] !== 'Background'
        );

        if (wearables.length !== 0) {
            const promises: Promise<any>[] = wearables.map((id: number) => TheGraphApi.getErc1155Price(id, true, Erc1155Categories.Wearable, 'timeLastPurchased', 'desc'));
